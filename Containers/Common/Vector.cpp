...
void KuszkAPI::Containers::Vector<tnData>::Sort(tnData ptData[], unsigned uCount, bool bGrow = true)
{
	...
	if (!bGrow) Containers::Vector<tnData>::Reverse(ptData, uCount);
}
...
template<typename tnData>
unsigned KuszkAPI::Containers::Vector<tnData>::Change(tnData ptData[], unsigned uSize, const tnData& tOld, const tnData& tNew, bool bAll)
{
	for (int i = 0; i < uSize; i++) if (ptData[i] == tOld){
