...
const HWND& KuszkAPI::Forms::TableBox::Create(int iXPos, int iYPos, int iSzerokosc, int iWysokosc, const Containers::Strings& sHeader, unsigned uStyl, unsigned uExStyl, const Containers::String& sClass)
{
     ...
     hUchwyt = CreateWindowEx(uExStyl | WS_EX_CLIENTEDGE, sClass.Str(), TEXT("ListView"), uStyl | WS_VISIBLE | WS_CHILD | LVS_SHAREIMAGELISTS | LVS_SHOWSELALWAYS, iXPos, iYPos, iSzerokosc, iWysokosc, hOwner, (HMENU) uId, hInstance, NULL);
...
void KuszkAPI::Forms::TableBox::Clean(void)
{
     CleanItems();
     CleanGroups();
     CleanHeader();
